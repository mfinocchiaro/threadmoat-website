...
| D006 |  | product | Free preview tier data visibility | Top 50 companies by weighted score, auto-selected. Permanent free access (no 30-day expiry). Free users see 3 charts + search/profiles for preview companies. | 50 companies is enough to prove product depth without giving away alpha. Auto-select by score is low-maintenance. Permanent access builds habit and email list without time-pressure resentment. | Yes | collaborative |
| D007 |  | product | Launch pricing strategy | Keep current pricing: Analyst $4,999 one-time, Strategist €18,999/yr. No founding discount. Analyst stays one-time purchase. | Signal premium positioning from day one. Lower pricing can always be offered via targeted coupons for strategic early customers without anchoring the public price low. | Yes | collaborative |
| D008 |  | ux | Company search UX pattern | Compact search bar integrated near the filter toolbar (not a separate page). Collapses/minimizes when not in use. Results open a company profile panel/page with curated expandable sections. | Keeps users in context — they can search without leaving their current chart. Avoids adding another sidebar nav item. Search bar near filters is a natural location since both are data-narrowing tools. | Yes | collaborative |
